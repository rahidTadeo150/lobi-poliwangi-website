...
<head>
    ...
    <title>Beasiswa Index | Lobi Poliwangi</title>
</head>
<body class="">
    ...
    <div class="pt-14">
        <div class="w-full h-[450px] relative">
            ...
            <div class="bg-gradient-to-l from-[rgba(0,0,0,0.55)] to-sky-600 absolute w-full h-full -translate-y-full flex flex-col justify-center items-center">
                <p class="text-3xl font-bold text-white -mt-6">Cari Beasiswa Yang Cocok Untukmu Disini.</p>
            </div>
            <div class="bg-slate-100 border border-slate-300 h-44 rounded-md shadow-md inset-x-32 -translate-y-1/2 absolute flex flex-row justify-between px-20 py-2 items-center">
                <div class="text-center">
                    <p class="text-xl font-semibold">Beasiswa Tersedia</p>
                    <p class="text-2xl font-semibold">{{ $jumlahBeasiswa }}</p>
                </div>
                <div class="text-center">
                    <p class="text-xl font-semibold">Beasiswa lokal</p>
                    <p class="text-2xl font-semibold">{{ $jumlahLokal }}</p>
                </div>
                <div class="text-center">
                    <p class="text-xl font-semibold">Beasiswa internasional</p>
                    <p class="text-2xl font-semibold">{{ $jumlahInternasional }}</p>
                </div>
            </div>
        </div>
        <div class="mt-32 px-16 pb-14">
            <p class="text-2xl font-bold mb-4">Informasi terbaru untukmu belakangan ini</p>
            <div class="overflow-hidden ml-4">
                @foreach ($beasiswaTerbaru as $dataTerbaru)
                <div class="w-80 h-80 bg-slate-200 float-left mr-20 relative rounded-md">
                    <div class="rounded-md opacity-0 hover:opacity-100 w-full h-full bg-[rgba(0,0,0,0.5)] absolute py-2 px-4 flex flex-col justify-center">
                        <p class="text-2xl font-semibold text-gray-100">{{ $dataTerbaru->nama_beasiswa }}</p>
                        <p class="text-lg font-semibold text-gray-100">{{ $dataTerbaru->instansiBeasiswa->nama_instansi_beasiswa }}</p>
            ...
            <p class="mt-10 text-xl font-bold">Daftar informasi beasiswa</p>
                ...
                <div class="w-3/6 mr-3 h-fit">
                    <form class="mb-0">
                        <div class="relative">
                            ...
                            <input type="search" class="block w-full px-4 py-3 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500" placeholder="Cari beasiswa disini" required>
            ...
            <div class="overflow-hidden ml-4 mt-4">
                @foreach ($beasiswaAll as $data)
                <div class="w-80 h-80 bg-slate-200 float-left mr-20 relative flex flex-col justify-end rounded-md">
                    <div class="bg-slate-100 w-full h-fit absolute py-4 px-2 rounded-b-md">
                        <p class="text-base font-semibold">{{ $data->nama_beasiswa }}</p>
                        <p class="text-sm font-semibold">{{ $data->instansiBeasiswa->nama_instansi_beasiswa }}</p>
                        <div class="flex flex-row justify-between">
                            <div></div>
                            <a href="/detail-beasiswa/{{ $data->id }}">
