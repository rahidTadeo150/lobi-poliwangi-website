...
@section('titlePage')
<title>{{ $dataInstansi->nama_instansi_lomba }} | Lobi Poliwangi</title>
@endsection

@section('mainArea')
<div class="w-full h-fit p-6 pt-14 pb-12 rounded-md bg-blue-500 flex flex-col justify-center items-center text-gray-100">
    <div>
        <p class="text-3xl font-bold">{{ $dataInstansi->nama_instansi_lomba }}</p>
    </div>
    <div class="mt-2">
...
<div class="w-full h-fit bg-slate-200 rounded-md mt-4 pl-12 pr-12 pb-8">
    <div class="h-full flex flex-row items-center">
        <div class="flex flex-col justify-center items-center">
            <p class="text-base font-semibold mb-4 mt-4">Postingan Terbaru Yang Terkait Dengan Instansi Ini</p>
            @if ($containData)
            <div class="w-80 h-80 rounded-md relative">
                <img src="{{ asset('/storage/' . $dataInstansi->lomba[0]->data_foto) }}" class="w-full h-full rounded-md">
                <div class="bg-[rgba(0,0,0,0.40)] backdrop-blur-[2px] absolute w-full h-full -translate-y-full opacity-0 p-3 pl-5 pr-5 hover:opacity-100">
                    <div class="w-full h-full flex flex-col justify-center">
                        <div>
                            <p class="text-xl w-full text-gray-100 font-semibold mb-4">{{ $dataInstansi->lomba[0]->nama_lomba }}</p>
                            <p class="text-base w-full text-gray-100 font-semibold">Dibuat Pada</p>
                            <p class="text-sm w-full text-gray-100 font-normal">{{ $dataInstansi->lomba[0]->created_at }}</p>
                            <p class="text-base w-full text-gray-100 font-semibold mt-4">Diupdate Pada</p>
                            <p class="text-sm w-full text-gray-100 font-normal">{{ $dataInstansi->lomba[0]->updated_at }}</p>
                            <a href="/dashboard-admin/index-data-lomba/preview/{{ $dataInstansi->lomba[0]->id }}">
        ...
        <div class="w-80 h-fit ml-32 pt-6 pb-6">
            ...
            @endif
                ...
                <a href="/dashboard-admin/index-data-instansi-lomba/edit-data/{{ $dataInstansi->id }}" class="text-sm w-fit h-fit p-3 bg-slate-300 rounded cursor-pointer bg-yellow-500 text-gray-100" type="submit"><span class="mr-2"><i class="fa-solid fa-pen-to-square"></i></span>Edit Data Ini</a>
                @if ($containData)
                <button class="w-fit h-fit p-3 bg-red-400 rounded cursor-pointer text-sm text-gray-200" type="submit" disabled><span class="mr-2"><i class="fa-solid fa-trash"></i></span>Nonaktifkan Data</button>
                @else
                <form action="/dashboard-admin/delete-data-instansi-lomba/{{ $dataInstansi->id }}" method="post">
                    @csrf
                    @method('delete')
                    <button class="w-fit h-fit p-3 bg-red-900 text-sm text-gray-200 rounded cursor-pointer" type="submit"><span class="mr-2"><i class="fa-solid fa-trash"></i></span>Nonaktifkan Data</button>
                </form>
                @endif
                @else
                <form action="/dashboard-admin/delete-data-instansi-lomba/{{ $dataInstansi->id }}" method="post">
